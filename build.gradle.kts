...
dependencies {
    ...
    ocfg("${sc.current.version}-fabric", "commands", "config", "config-impl", "events", "internal", "ui", "utils", "hud", "notifications")

    modImplementation("net.fabricmc.fabric-api:fabric-api:${sc.properties["deps.fabric_api"] as String}")

    // Compose classes ship at runtime via OneConfig's compose-bundle (pulled by :internal);
    // declared compileOnly so this mod's @Composable code resolves against the same versions.
    val compose = "1.12.0-alpha01"
    ...
    compileOnly("org.jetbrains.compose.ui:ui-util-desktop:$compose")
}
